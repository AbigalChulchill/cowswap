...
// MOD
import { AMOUNT_PRECISION, FIAT_PRECISION, INITIAL_ALLOWED_SLIPPAGE_PERCENT } from 'constants/index'
...
export default function Limit({
  ...
  const priceSide = useMemo(
    () =>
      trade &&
      tryParseAmount(
        trade?.executionPrice.toFixed(trade?.executionPrice.quoteCurrency.decimals),
        trade?.executionPrice.quoteCurrency
      ),
    [trade]
  )
  formattedAmounts[Field.OUTPUT] = useMemo(() => {
    if (!limitPrice || !trade || !priceSide) return formattedAmounts[Field.OUTPUT]
              ...
              */}
              ...
              <AutoRow padding={'20px 0'} margin={'0px'} gap="20px">
                <Input
                  ...
                  placeholder={priceSide?.toExact() ?? 'price'}
                  value={limitPrice}
